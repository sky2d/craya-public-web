...
const OrderProductCard: React.FC<ProductCardProps> = props => {
  return (
    <>
...
const ProductContent: React.FC<ProductCardProps> = props => {
  const { description, price, discountedPrice, name } = props.item.product;
  const { createdAt, estimatedDeliveryDate, paymentStatus, deliveryStatus } = props.order;

  const image = props.item.productSKU.images[0];
      ...
      <div className="flex h-full w-full flex-col items-stretch justify-start p-4">
        ...
        <div className="flex h-full items-start justify-between">
          <div className="flex flex-col items-center text-center">
            ...
            <span className="mt-1 flex aspect-square items-center justify-center rounded-full bg-brand-color1 p-1 text-sm text-white-light4">
