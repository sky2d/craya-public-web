...
export const ShoppableVideoFeedConfig: React.FC<StorefrontComponentConfigProps> = ({ data, setData }) => {
      ...
      {data.loops.map((loop, index) => (
        <div key={index} className="rounded-2xl border-2 border-brand-color1 p-2">
          ...
          <div className="container flex gap-2">
            ...
            <div className="products flex flex-col justify-between">
              {loop.products.slice(0, 3).map((item, id) => (
                <div key={id} className="h-16 w-16">
                  <img className="rounded-t-lg" src={item.image?.fileUrl} alt={item.name} />
                  <div className="flex h-auto items-center justify-center rounded-b-lg bg-black-dark1 text-white-light4">
                    <span className="body-xs">View Product</span>
                  </div>
                </div>
              ))}
            </div>
